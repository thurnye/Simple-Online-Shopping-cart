...
export class SalesforceCartClientDouble {
 ...
 getContext(id: string): Context {
  ...
  priceCart(items: CartItem[]): { items: PricedItem[]; totals: Totals; currency: string } {
  ...
  private expired(id: string, when?: number) {
    const detail = when
      ...
      : `Context ${id} expired`;
    return Object.assign(new Error(detail), { code: 409, type: "context_expired" });
  }

  private notFound(id: string) {
    return Object.assign(new Error(`Context ${id} not found`), { code: 404, type: "context_not_found" });
  }

  private unprocessable(detail: string) {
    return Object.assign(new Error(detail), { code: 422, type: "unprocessable" });
  }
}

export const salesforce = new SalesforceCartClientDouble();
